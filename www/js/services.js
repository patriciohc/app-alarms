...
.factory('sharedConn', ['$ionicPopup','$state','$rootScope','$ionicPopup', function($ionicPopup, $state, $rootScope , $ionicPopup ){
     ...
     SharedConnObj.WSS_SERVICE = 'wss://suchat.org:5281/websocket';
     SharedConnObj.BOSH_SERVICE = 'http://suchat.org:5222/http-bind/';
     SharedConnObj.connection   = null;    // The main Strophe connection object.
     SharedConnObj.loggedIn = false;
     SharedConnObj.statusConnection = Strophe.Status.DISCONNECTED;
    ...
    SharedConnObj.login=function (jid,host,pass) {
        SharedConnObj.connection = new Strophe.Connection( SharedConnObj.WSS_SERVICE , {'keepalive': true});  // We initialize the Strophe connection.
        SharedConnObj.connection.connect(jid+'@'+host, pass , SharedConnObj.onConnect);
    };

    //On connect XMPP
    SharedConnObj.onConnect=function(status){
        SharedConnObj.statusConnection = status;
        ...
        $rootScope.$broadcast('statusConnection',  status);
    };
